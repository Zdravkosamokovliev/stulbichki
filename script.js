document.addEventListener('DOMContentLoaded', () => {
    ...
    let tooltipElement = null; // Променлива за динамичния тултип

    // --- КОМУНИКАЦИЯ И ПОМОЩНИ ФУНКЦИИ ---
    ...
    function initializeMasterRoadmap() {
        elements.masterProgressBarBG.style.width = `${MASTER_BAR_WIDTH_PX}px`;
        const maxFloors = checkpoints[checkpoints.length - 1].totalFloors;
        elements.masterCheckpointsContainer.innerHTML = '';
        checkpoints.forEach(cp => {
            const marker = document.createElement('div');
            marker.className = 'checkpoint-marker';
            const leftPositionPx = (cp.totalFloors / maxFloors) * MASTER_BAR_WIDTH_PX;
            marker.style.left = `${leftPositionPx}px`;
            marker.dataset.name = `${cp.name} (${cp.altitude} м)`;
            
            // --- НОВА ЛОГИКА ЗА ТУЛТИПА ---
            marker.addEventListener('mouseenter', (e) => {
                tooltipElement = document.createElement('div');
                tooltipElement.className = 'dynamic-tooltip';
                tooltipElement.textContent = e.target.dataset.name;
                document.body.appendChild(tooltipElement);

                // Позиционираме го спрямо мишката
            ...
            elements.masterCheckpointsContainer.appendChild(marker);
        });
    }
